...
fn main() -> io::Result<()> {
  ...
  let matches = Command::new("fill-extension-flow-stat")
      .version("0.1")
      .author("Lucas Aubard <lucas.aubard@irisa.fr>")
      .about("Fill extension flow stat field in three-tuple-frag-stat logs")
      .arg(
        Arg::new("log-path")
        .short('p')
        .long("log-path")
  ...
  T3StatC::fill_flow_stat_extension_field(&mut t3_stat_c);
